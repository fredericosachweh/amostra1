...
class Test_XML_to_db(object):
	...
	def test_Programme_1(self):
		...
		self.assertItemsEqual(programme.titles.values_list('lang__value', 'value'), [(u'pt', u'BBC Sessions: The Verve'),(u'en', u'BBC Sessions: Verve; The')])
		self.assertEquals(programme.descriptions.values_list('lang__value','value')[0], (None, u'Uma impressionante atuação do The Verve no famoso estúdio Maida Vale, da BBC. Desfrute desta íntima, porém poderosa gravação da banda de Richard Ashcroft, que inclui músicas como Bitter Sweet Symphony e Love is Noise. - www.revistaeletronica.com.br '))
...
class One_Raw_XML(Test_XML_to_db, TestCase):
	...
	def test_Programmes_count(self):
...
class Two_Zipped_XMLs(Test_XML_to_db, TestCase):
	...
	def test_Epg_Source(self):
		...
		self.assertEquals(self.epg_source.numberofElements, 4)
	...
	def test_Programme_2(self):
		...
		self.assertEquals(programme.source, self.epg_source)
		self.assertItemsEqual(programme.titles.values_list('lang__value', 'value'), [(u'pt', u'Três Homens em Conflito'),(u'en', u'The Good, The Bad and the Ugly')])
		self.assertEquals(programme.descriptions.values_list('lang__value','value')[0], (None, u'Durante a Guerra Civil Americana, três aventureiros tentam pôr as mãos numa fortuna. - www.revistaeletronica.com.br '))
		...
		self.assertItemsEqual(programme.actors.values_list('name'), ( ((u'Clint Eastwood'),), ((u'Lee Van Cleef'),), ((u'Eli Wallach'),), ((u'Rada Rassimov'),), ((u'Mario Brega'),), ))
