...
class APITest(TestCase):
    ...
    def test_Channel_REST(self):
        c = Client()
        urlchannel = reverse('api_dispatch_list',
            kwargs={'resource_name': 'channel', 'api_name': 'epg'})
        self.assertEqual(urlchannel, '/tv/api/epg/channel/')
        response = c.get(urlchannel)
        #print(response)
        expected = [\
{'display_name': 'Concert Channel', 'icons': [{'src': '100.png', 'id': '1', \
    'resource_uri': '/tv/api/epg/icon/1/'}], 'channelid': '100', 'urls': [], \
    'id': '1', 'resource_uri': '/tv/api/epg/channel/1/'},
{'display_name': 'Band HD', 'icons': [{'src': '505.png', 'id': '2', \
    'resource_uri': '/tv/api/epg/icon/2/'}], 'channelid': '505', 'urls': [], \
    'id': '2', 'resource_uri': '/tv/api/epg/channel/2/'}
            ]
        ...
        test_cases = (
            { 'expected' : expected,
              'requests' : (('/tv/api/epg/channel/', {}),
                              ('/tv/api/epg/channel/', {}),
                              ('/tv/api/epg/channel/1;2/', {}),
                              ('/tv/api/epg/channel/1;2', {}),
              )
            },
            { 'expected' : [expected[1]],
              'requests' : (('/tv/api/epg/channel/', {'channelid' : '505'}),
              )
            },
        )
        ...
        for test in test_cases:
            for request in test['requests']:
                ...
                jobj = json.loads(response.content)
                objects = jobj['objects']
                self.assertEquals(objects, test['expected'], msg=response.request)
        # Check for 404 if resource doesn't exists
        response = c.get('/tv/api/epg/channel/3/')
        self.assertEquals(response.status_code, 404)

    def test_Programme_REST(self):
        c = Client()
        response = c.get('/tv/api/epg/programme/')
