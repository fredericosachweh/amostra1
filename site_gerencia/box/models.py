...
class ToggleSignal():
    ...
    execuções desnecessárias no acesso de instancias filho que contenham um
    signal connectado a ela.

    !CUIDADO!
    Não vai desconectar e após o processo esquecer de reconectar, senão ele
    perde o sinal apartir do momento que sai do processo. """
    _instance = None

    def __new__(cls, *args, **kwargs):
        """ Singleton """
        if not cls._instance:
            cls._instance = super(ToggleSignal, cls).__new__(
                cls, *args, **kwargs)
...
def _pre_save_push_server_command(signal, instance, sender, **kwargs):
    ...
    if isinstance(instance, SetupBoxCommands):
        ...
        def _post_save_push_server_command(signal, instance, sender, **kwargs):
            ...
            for setupbox in SetupBox.objects.filter(
                    pushserver=instance.pushserver, connected=True):
                online = PushServerCommandsSetupBox(
                    pushservercommands=instance, setupbox=setupbox)
                online.save()
        models.signals.post_save.connect(_post_save_push_server_command,
