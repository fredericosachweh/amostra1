...
urlpatterns = patterns('',
    ...
    url(r'^channels/(?P<channel_ids>(\d+(,)*)+)/?$', channel_handler, name='channel_handler'),
    ...
    url(r'^programmes/(?P<programme_ids>(\d+(,)*)+)/?$', programme_handler, name='programme_handler'),
    url(r'^programmes/(?P<programme_ids>\d+)/(?P<fields>((%s)(,)*)+)/?$' % programme_filters, programme_handler),

    # REST interface for guide resources
    url(r'^guide/?$', guide_handler),
    url(r'^guide/(?P<ids>(\d+(,)*)+)/?$', guide_handler, name='guide_handler'),
