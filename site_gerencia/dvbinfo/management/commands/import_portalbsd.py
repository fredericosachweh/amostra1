...
class Command(BaseCommand):
    ...
    help = 'Automatically fetch and import tunning info data from PortalBSD\'s website'
    ...
    def _create_satellite(self, sat_soup, sat_name, sat_location):
        ...
        res = re.findall(u'(Bandas:.*)(Polarização:.*)(Capacidade:.*)(Lançamento:.*)(Vida Útil:.*)', info)
        if len(res) > 0:
    ...
    def _create_transponder(self, tr, band, satellite):
        ...
        # Get tunning info
        res = re.findall('^([0-9]*) (.)([0-9]*) \| (\?|[0-9]/[0-9])(.*)$', td[0].text)
        if len(res):
            freq = res[0][0]; pol = res[0][1]; sr = res[0][2]; fec = res[0][3]; domain = res[0][4]
        else:
        ...
        transponder, created = Transponder.objects.get_or_create(name=name,band=band, \
                                frequency=freq,symbol_rate=sr,polarization=pol,fec=fec, \
                                system=system,modulation=modulation,logo=logo,satellite=satellite)
        return transponder
    ...
    def _create_channel(self, tr, transponder):
        ...
        if len(c) is 5:
            ...
            try:
                ...
                last_update = date(int(res[0][2]), int(res[0][1]), int(res[0][0]))
    ...
    def handle(self, *args, **kwargs):
        ...
        for sat in soup.find('select', attrs={'name': 'canal'}).findAll('option'):
            if sat['value']:
                results = re.findall('(.*) - (.*)', sat.string)
                sat_location = results[0][0]
                sat_name = results[0][1]
                
                # Input arguments: if satellites are provided as args we only import them
                if len(args):
                ...
                self.stdout.write('Fetching %s - %s\n' % (sat_location, sat_name))
                sat_url = re.findall('parent\.location\.href=\'(.*)\';', sat['value'])
                sat_soup = self._get_soup(self.base_url + sat_url[0])

                # Create Satellite object
                satellite = self._create_satellite(sat_soup, sat_name, sat_location)
                ...
                for index in range(0, len(tables), 2):
                    # Get band
                    text = tables[index].find('tr').find('td', attrs={'class' : None}).text
                    if text.startswith(u'Canais do satélite'):
                        res = re.findall(u'^Canais do satélite .* - Banda (.*)$', text)
                        band = res[0]
                    ...
                    while i < len(rows):
                        ...
                        # Case 1: Mux with PAC link - follow link and skip channels above if any
                        if td[4].find('img', alt='Ver pacote de canais'):
                            url_mux = self.base_url + 'satelite_canais.php' + td[4].find('img', alt='Ver pacote de canais').parent['href']
                            # Fetch Mux
                            ...
                            if len(mux_tables) is not 2:
                                continue
                            mux_rows = mux_tables[1].findAll('tr')[1:]  # Skip tables's header
                            # Create the transponder
                            tp = self._create_transponder(mux_rows[0], band, satellite)
                            if not isinstance(tp, Transponder):
                                continue
                            # Create the channels
                            [self._create_channel(row, tp) for row in mux_rows if isinstance(tp, Transponder)]
                                
                            # Forward to next transponder
                            while (i+1) < len(rows) and len(rows[i+1].findAll('td', {'class' : None})[0].text) is 0:
                                i = i + 1
                        # Case 2: Transponder with one (or multiple) channels - Get all channels
                        else:
                            # Create the transponder
                            tp = self._create_transponder(rows[i], band, satellite)
                            # Create the channels
