...
class Server(models.Model):
    ...
    def auto_create_nic(self):
        ...
        for iface in self._list_interfaces():
            ...
            if nnics == 0:
                nic = NIC.objects.create(name=iface['dev'], server=self, ipv4=iface['ip'])
            else:
...
class Dvblast(DeviceServer):
    ...
    def status(self):
        ...
        if p.is_playing(self) is True:
            ...
            return '<a href="%s" id="stream_id_%s" style="color:green;" >Rodando</a>' % (url, self.id)
...
class DvbTuner(DigitalTuner):
    ...
    def adapter_num(self):
        import re
        files = self.server.execute(
            '/bin/find /dev/dvb/ -name adapter*.mac -type f', persist=True)
        for file in files:
...
class FileInput(DeviceServer):
    ...
    def switch_link(self):
        ...
        if self.running():
            ...
            return '<a href="%s" id="vlc_id_%s" style="color:green;" >Rodando</a>' % (url, self.id)
